...
class BaseVisualizationMixin:
    """Mixin class containing common visualization methods adapted for PyQt5"""
    ...
    def initialize_common_variables(self):
        ...
        # Polar plot axis mapping
        self.polar_x_axis = 'Yaw'
        self.polar_y_axis = 'Pitch'
        ...
        self.polar_update_frequency = 1
        self.blur_counter = 0
        self.blur_frequency = 5
        
        # Y-axis range parameters (adjusted for voltage in V, e.g. -0.5 to 0.5)
        self.y_axis_min = -0.5
        self.y_axis_max = 0.5
    ...
    def setup_window_sizing(self, window_widget):
        ...
        window_widget.setMinimumSize(800, 600)
    ...
    def initialize_weighting_array(self, distance):
        ...
        if len(distance) > self.front_plot_threshold:
            ...
            start_idx = 500
            end_idx = len(distance_cropped) - 500
    ...
    def setup_left_controls(self, layout):
        ...
        self.y_axis_min_input.setPlaceholderText("Y-axis Min (e.g., -0.5)")
        ...
        self.y_axis_max_input.setPlaceholderText("Y-axis Max (e.g., 0.5)")
        ...
        self.x_axis_label.setFixedSize(self.control_width, 40)
        self.x_axis_label.setStyleSheet(f"font-size: {self.font_size - 5}px; color: black; padding: 5px;")
        layout.addWidget(self.x_axis_label, row, 0)
        row += 1
        
        self.x_axis_combo = QtWidgets.QComboBox()
        self.x_axis_combo.addItems(['Roll', 'Pitch', 'Yaw'])
        self.x_axis_combo.setCurrentText(self.polar_x_axis)
        self.x_axis_combo.setFixedSize(self.control_width, self.control_height)
        self.x_axis_combo.setStyleSheet(f"font-size: {self.font_size + 5}px; padding: 10px;")
        layout.addWidget(self.x_axis_combo, row, 0)
        row += 1
        
        # Y-Axis Source Selection
        self.y_axis_label = QtWidgets.QLabel('Polar Y-Axis:')
        self.y_axis_label.setFixedSize(self.control_width, 40)
        self.y_axis_label.setStyleSheet(f"font-size: {self.font_size - 5}px; color: black; padding: 5px;")
        layout.addWidget(self.y_axis_label, row, 0)
        row += 1
        
        self.y_axis_combo = QtWidgets.QComboBox()
        self.y_axis_combo.addItems(['Roll', 'Pitch', 'Yaw'])
        self.y_axis_combo.setCurrentText(self.polar_y_axis)
        self.y_axis_combo.setFixedSize(self.control_width, self.control_height)
        self.y_axis_combo.setStyleSheet(f"font-size: {self.font_size + 5}px; padding: 10px;")
        layout.addWidget(self.y_axis_combo, row, 0)
    ...
    def setup_main_plot(self, layout):
        ...
        axis.setStyle(tickFont=pg.QtGui.QFont('Arial', self.font_size + 10))
        ...
        wavelet_axis.setStyle(tickFont=pg.QtGui.QFont('Arial', self.font_size + 10))
        ...
        self.wavelet_plot_widget.setYRange(0, 0.5)
    ...
    def setup_polar_plot(self, layout):
        ...
        # Static crosshairs
        static_pen = pg.mkPen(color=(150, 150, 150, 120), width=2, style=Qt.DotLine)
    ...
    def setup_common_connections(self):
        ...
        self.y_axis_combo.currentTextChanged.connect(self.on_y_axis_changed)

    def on_x_axis_changed(self, text):
        self.polar_x_axis = text

    def on_y_axis_changed(self, text):
        self.polar_y_axis = text
    ...
    def main_update(self):
        ...
        if dt > 0.001:
            self.refresh_rate_array.append(1.0 / dt)
    ...
    def process_segmentation(self, wavelet_data, wavelet_distance):
        ...
        try:
            ...
            # Scaled for voltage bounds on Pitaya
            self.current_segmentation_intensity = min(1.0, self.current_segmentation_intensity / 5.0)
    ...
    def calculate_tip_position(self, relative_pitch, relative_roll):
        """Calculate tip position from pitch and roll angles"""
        d = 2
        pitch_rad = np.radians(relative_pitch)
        roll_rad = np.radians(relative_roll)
        
        y = d * np.sin(roll_rad) * np.cos(pitch_rad)
    ...
    def apply_colormap(self, img_array, cmap_name='jet'):
        """Apply colormap to image array with caching"""
    ...
    def update_save_data_button_state(self):
        """Update save data button appearance"""
    ...
    def on_front_threshold_changed(self, value):
        """Handle front threshold changes"""
    ...
    def on_y_axis_min_changed(self, text):
        """Handle Y-axis minimum changes"""
        try:
            self.y_axis_min = float(text)
            self.plot_widget.setYRange(self.y_axis_min, self.y_axis_max)
    ...
    def on_y_axis_max_changed(self, text):
        """Handle Y-axis maximum changes"""
        try:
            self.y_axis_max = float(text)
            self.plot_widget.setYRange(self.y_axis_min, self.y_axis_max)
