...
class LiveScope(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.x_axis_combo.setCurrentText('Yaw')
        ...
        self.y_axis_combo.setCurrentText('Pitch')
    ...
    def update(self):
        ...
        try:
            ...
            if args.no_imu:
                ...
                # Stability Logic
                quat_array = np.array([qx, qy, qz, qw]) # scipy uses x,y,z,w
                ...
                # print(f"RAW QUAT -> W:{qw:.6f} (int:{qw*16384:.1f}) X:{qx:.6f} (int:{qx*16384:.1f}) Y:{qy:.6f} (int:{qy*16384:.1f}) Z:{qz:.6f} (int:{qz*16384:.1f}) | NORM:{norm:.4f}")
                ...
                if abs(norm - 1.0) < 0.1:
                    try:
                        ...
                        if self.initial_quat is None:
                            ...
                            # Jump check (10 degrees in 8ms is 1250 deg/s, definitely a glitch) - DISABLED
                            # if abs(relative_pitch - self.pitch_angles[-1]) > 10.0 or abs(relative_yaw - self.yaw_angles[-1]) > 10.0 or abs(relative_roll - self.roll_angles[-1]) > 10.0:
                            #     print(f"JUMP DETECTED: Pitch diff {abs(relative_pitch - self.pitch_angles[-1]):.1f}, Yaw diff {abs(relative_yaw - self.yaw_angles[-1]):.1f}, Roll diff {abs(relative_roll - self.roll_angles[-1]):.1f}. Skipping frame.")
                            #     # Keep the previous angles by not updating self.pitch_angles, roll_angles, yaw_angles
                            # else:
                            if True:
                                ...
                                # DEBUG: Print values to terminal - DISABLED
                                # print(f"IMU Angles -> Yaw: {relative_yaw:6.1f} | Roll: {relative_roll:6.1f} | Pitch: {relative_pitch:6.1f}")
            ...
            if num_pulses > 0:
                ...
                # Update Plot
                self.curve1.setData(x=self.x_data, y=avg_pulse)
