...
from PyQt6.QtWidgets import QComboBox, QVBoxLayout, QWidget, QLabel, QSlider
...
class TimeSeriesPlotter(QWidget):
    def __init__(self, title="Time Series Plot"):
        ...
        self.plot = self.win.addPlot(row=1, col=0, viewBox=CustomViewBox(self))
        ...
        # Load subdirectories
        self.top_level_directory = '/Volumes/Samsung_T5/SoundPass_datasets/new_datasets/2025-09-15' # "/Volumes/Samsung_T5/SoundPass_datasets/new_datasets/2025-09-15"
        self.directories = [
    ...
    def directory_changed(self, idx):
        ...
        self.current_index = 0
        self.slider.setMaximum(len(self.file_names) - 1)  # Update the slider range
        self.slider.setValue(0)
        self.plot_data()

    def slider_changed(self, value):
        """
        Handle changes to the file index via the slider.
        """
        self.current_index = value
        self.plot_data()

    def load_data(self):
        if not self.all_data_array or self.current_index >= len(self.all_data_array):
            return
    ...
    def key_pressed(self, evt):
        if evt.key() == Qt.Key.Key_Space:
            ...
            if next_index >= len(self.file_names):
                ...
                # Move to the next file in the current directory
