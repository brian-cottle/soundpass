...
class SegmentationDataset(Dataset):
    def __init__(self,
                 file_paths=['/home/briancottle/Code/SoundPass/datasets/phantom_annotated/2025-03-20_ground_truth',
                             '/home/briancottle/Code/SoundPass/datasets/phantom_annotated/2025-03-22_ground_truth',
                             '/home/briancottle/Code/SoundPass/datasets/phantom_annotated/2025-03-24_ground_truth',
                             '/home/briancottle/Code/SoundPass/datasets/phantom_annotated/2025-03-25_ground_truth',
                             '/home/briancottle/Code/SoundPass/datasets/phantom_annotated/2025-03-26_ground_truth',],
                 split='Train',
                 percentage=1.0,
                 baseline=False,
                 normalize=True,
        ...
        for file_path in file_paths:
            pkl_files = glob(f"{file_path}/*.pkl")
            all_file_names.extend(pkl_files)

        split_idx = int(0.8 * len(all_file_names))
        val_idx = int(0.9 * len(all_file_names))
...
class ClassificationNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        x = self.conv2(x)
        x = self.global_avg_pool(x).squeeze(-1)  # Reduce to (batch_size, num_classes)
...
train_dataset = SegmentationDataset(percentage=percentage, split='Train', baseline=True, downsample_rate=downsample_rate)
dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

# Validation dataset and DataLoader
val_dataset = SegmentationDataset(split='Val',percentage=percentage, baseline=True, downsample_rate=downsample_rate)
val_dataloader = DataLoader(val_dataset, batch_size=batch_size, shuffle=True) 

# Test dataset and DataLoader   
test_dataset = SegmentationDataset(split='Test',percentage=percentage, baseline=True, downsample_rate=downsample_rate)
...
criterion = nn.CrossEntropyLoss(weight=torch.tensor([ratio*10, (1-ratio)*10]).float())  # Adjust weights for class imbalance
...
save_dir = '/home/briancottle/Code/SoundPass/model_weights/v0_2'
...
model_weights_file = os.path.join(save_dir, "model_epoch_33.pth")  # Replace with the desired file name
