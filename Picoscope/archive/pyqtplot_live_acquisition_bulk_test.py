...
class TimeSeriesPlotter:
    def __init__(self):
        ...
        self.btn = QtWidgets.QPushButton('start gathering data')
        self.qt_connections()

        layout.addWidget(self.btn, 0, 0)  # button goes in upper-left
        layout.addWidget(self.plot_widget, 0, 1, 3, 1)  # plot goes on right side, spanning 3 rows
        ...
        now = datetime.now()
        self.save_directory = now.strftime("%Y-%m-%d_%H-%M-%S")
    ...
    def update_plot(self):
        ...
        baselined_samples = average_samples - self.baseline_array


        if self.save_data:
            now = datetime.now()
            formatted_date_time = int(time.time()*1000)
            file_name = f"{self.sub_directory}/{formatted_date_time}.pkl"
        ...
        windowed_samples = np.zeros(len(average_samples))
        windowed_filtered = sliding_window_view(average_samples, window_shape=window_size)
        ...
        self.y2_data = baselined_samples
        # Update plot
        self.plot1.setData(self.x_data, self.y1_data)
        self.plot2.setData(self.x_data, self.y2_data)
